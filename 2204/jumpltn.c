...
err2204_t jumpltn2204(cpu_t *cpuDevice, ram_t *ramDevice, char *debugString)
{
    ...
    /* Get jump address */
    cpuDevice->PC++;
    sprintf(debugString, "Getting jump backward distance.");
    debug(debugString, INFO);
    ...
    sprintf(debugString, "Jump backward distance %" PRIu64, cpuDevice->arguments[2]);
    debug(debugString, INFO);
    
    /* Check if the two operands fulfil the jump condition */
