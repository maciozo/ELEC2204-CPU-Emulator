...
err2204_t cpuRun(cpu_t *cpuDevice, ram_t *ramDevice)
{
    ...
    while(1)
    {
        ...
        switch (cpuDevice->currentInstruction)
        {
            ...
            case JEQA:
                sprintf(debugString, "Instruction: JEQA");
                debug(debugString, INFO);
                error = jumpeq2204(cpuDevice, ramDevice, debugString);
                if (error.errnum != SUCCESS)
                {
                    return (error);
                }
            ...
            case JEQP:
                sprintf(debugString, "Instruction: JEQP");
                debug(debugString, INFO);
                error = jumpeqp2204(cpuDevice, ramDevice, debugString);
                if (error.errnum != SUCCESS)
                {
                    return (error);
                }
            ...
            case JLTP:
                sprintf(debugString, "Instruction: JLTP");
                debug(debugString, INFO);
                error = jumpltp2204(cpuDevice, ramDevice, debugString);
                if (error.errnum != SUCCESS)
                {
                    return (error);
                }
